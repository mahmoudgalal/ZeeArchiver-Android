...
STDMETHODIMP CExtractCallbackImp::PrepareOperation(const wchar_t *name, bool isFolder, Int32 askExtractMode/* askExtractMode */, const UInt64 * position/* position */)
{
...
{
	jstring Name =enviro.env->NewStringUTF((LPCSTR)GetOemString(name));
	enviro.env->CallVoidMethod(enviro.obj,openResult,Name,(jlong)result,encrypted?JNI_TRUE:JNI_FALSE);
	enviro.env->DeleteLocalRef(Name);
...
//    Password = dialog.Password;
	  jstring pass=(jstring)enviro.env->CallObjectMethod(enviro.obj,cryptoGetTextPassword,NULL);
	  if(pass==NULL)
		  return E_ABORT;
	  char outbuf[255];
